...
from .SkinUtils import getScalingFactor
...
class List(GUIComponent):
	...
	def __init__(self):
		...
		# set default values if not set by widget in skin
		scaling_factor = getScalingFactor()
		self.list.setFont(0, gFont("Regular", int(round(30 * scaling_factor))))
		self.list.setItemHeight(int(round(40 * scaling_factor)))
	...
	def buildList(self, entry):
		...
		res.append((eListboxPythonMultiContent.TYPE_TEXT, 5, 0, width, height, 0, RT_HALIGN_LEFT | RT_VALIGN_CENTER, title))
		return res
	...
	def getCurrent(self):
		cur = self.list.getCurrentSelection()
